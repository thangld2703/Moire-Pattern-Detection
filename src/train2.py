...
def readWaveletData(positiveImagePath, negativeImagePath, positiveTrainImagePath, negativeTrainImagePath):
    ...
    sampleIndex = readImageSet(positiveImageFiles, positiveTrainImagePath, X_LL, X_LH, X_HL, X_HH, X_index, Y,
                               sampleIndex, 0)
    print('positive data loaded.')

    sampleIndex += readImageSet(negativeImageFiles, negativeTrainImagePath, X_LL, X_LH, X_HL, X_HH, X_index, Y,
                                sampleIndex, 1)
...
def trainCNNModel(X_LL_train, X_LH_train, X_HL_train, X_HH_train, y_train,
    ...
    model.fit([X_LL_train, X_LH_train, X_HL_train, X_HH_train], Y_train,  # Train the model using the training set...
              ...
              callbacks=callbacks_list)  # ...holding out 10% of the data for validation
    score, acc = model.evaluate([X_LL_test, X_LH_test, X_HL_test, X_HH_test], Y_test,
